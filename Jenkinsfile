                ...
                container('tools') {
                    script {
                        ...
                        openshift.withCluster() {
                            openshift.withProject() {
                                // Validando
                                if (!openshift.selector("deploy", "${APP_NAME}-${AMBIENTE}").exists()){
                                    ...
                                    def dc = openshift.selector("deploy", "${APP_NAME}-${AMBIENTE}")
                                    ...
                                    def app = openshift.apply("--file=./k8s/template.yaml", "--param=APP_NAME=${APP_NAME}-${AMBIENTE}", "--param=APP_VERSION=${APP_VERSION}", "--param=AMBIENTE=${AMBIENTE}", "--param=REGISTRY=${PUSH}:${APP_VERSION}-${AMBIENTE}" )
                                }
                            }
                        }
                    }
                    ...
                    script {
                        openshift.withCluster() {
                            openshift.withProject(){
                                ...
                                if (openshift.selector("deploy", "${APP_NAME}-${AMBIENTE}").exists()){
                                    //def latestDeploymentVersion = openshift.selector('deploy',"${APP_NAME}-${AMBIENTE}").object().status.latestVersion
...
def rollback(){
    ...
    rollback = openshift.selector("deploy/${APP_NAME}-${AMBIENTE}").rollout().undo("--to-revision=${REVISION}")
